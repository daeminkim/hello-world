...
- 조인의 종류
    ...
    - Outer Join
        - 한쪽 테이블(소스테이블)의 행들을 모두 사용하고 다른 쪽 테이블은 조인 조건을 만족하는 행만 합친다. 조인조건을 만족하는 행이 없는 경우 NULL을 합친다.
...
--TODO 부서별 급여등급이(salary_grade.grade) 1인 직원있는 "부서별로 1등급 부서이름(dept.dept_name)과 1등급인 직원수 조회."" 직원수가 많은 부서 순서대로 정렬.
