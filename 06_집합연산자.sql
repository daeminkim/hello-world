/* **********************************************************************************************
집합 연산자 (결합 쿼리)
- 둘 이상의 select 결과를 가지고 하는 연산.  select select  연산 
- 구문
...
-연산자
 ...
 - 규칙
  ...
  - 연산대상 select 문의 컬럼의 타입이 같아야 한다.  -- 데이터 타입  // 같은 번째? 의 컬럼의 타입 같으면됨  
  - 연산 결과의 컬럼이름은 첫번째 select문의 것을 따른다.
  - order by 절은 구문의 마지막에 넣을 수 있다.   -- 테이블이 다만들어 지면 결과를 정리하는거니까~~
  - UNION ALL을 제외한 나머지 연산은 중복되는 행은 제거한다.   // 두개의 테이블을 합치는 것과 같으니까 컬럼 타입 개수 같아야한다. 컬럼명은 제일 첫번째거
...
select*from emp where dept_id in(10,20)
union   -- 중복 없을때는  union all 로 하는게 맞다.// 나머지들은 마지막에 중복 체크를 하기 때문에 union all 로하는게 더 좋다.
...
select e.item
from export_rank e
where  e.year = '2018' --and ranking = 1
intersect
select i.item
from import_rank i
where i.year = '2018'; --and ranking = 1;
...
--union all
select '수입' label , e.item
from export_rank e
where e.year = '1990'
union all
select '수출' label , i.item
from import_rank i
where i.year = '1990';
...
select e.item    -- 품목만 보고 싶을때  앞에 라벨이 있으면 라벨때메 아이템이 같은게 결과테이블로 나온다.
from export_rank e
where e.year = '1990'
union 
select i.item
